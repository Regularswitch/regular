...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
	...
	if (!section) {
		return buildPageMetadata({}, { fallbackTitle: 'RegularSwitch', locale });
	}

	const [seo, fallback] = await Promise.all([
		fetchSectionSeo(section, locale),
		Promise.resolve(sectionSeoFallbacks(section, locale)),
	]);
...
export default async function SlugPage({ params }: PageProps) {
	...
	if (slug === CAPABILITIES_PAGE_SLUG) {
		...
		const faqJsonLd = buildFaqPageJsonLd(
			content.faq,
			locale === 'pt' ? '/PT/capabilities' : '/capabilities',
		);

		return (
			<>
				<JsonLd id="capabilities-faq-jsonld" data={faqJsonLd} />
				<CapabilitiesPage content={content} latestProjects={latestProjects} locale={locale} />
			</>
		);
	}
