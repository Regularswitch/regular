...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
	...
	if (!slug || slug === HOME_PROJECTS_CATEGORY_SLUG) {
		return buildPageMetadata({}, { fallbackTitle: DEFAULT_SITE_NAME, locale: 'pt' });
	}

	const data = await fetchCategoryArchivePage(slug, 'pt').catch(() => null);
	if (!data) {
		return buildPageMetadata({}, { fallbackTitle: DEFAULT_SITE_NAME, locale: 'pt' });
	}
	...
	return buildPageMetadata(
		...
		},
		{
			...
			locale: 'pt',
			path: `/PT/category/${slug}`,
		},
	);
}

export default async function PtCategorySlugPage({ params }: PageProps) {
	const { slug } = await params;
	if (!slug || slug === HOME_PROJECTS_CATEGORY_SLUG) notFound();

	const data = await fetchCategoryArchivePage(slug, 'pt').catch((error) => {
		console.error('Error fetching PT category archive', error);
		return null;
	});
